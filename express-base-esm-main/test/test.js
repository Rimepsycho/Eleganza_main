...
const testSql = async () => {
  const r1 = await executeQuery(`INSERT INTO products 
                                 VALUES (NULL, "商品名", 999, "")`)
  console.log(r1.rows)
  console.log(r1.rows.insertId)

  const pid = r1.rows.insertId

  const r2 = await executeQuery(`SELECT * FROM products WHERE id=${pid}`)
