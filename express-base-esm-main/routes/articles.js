...
router.get('/search', async (req, res) => {
  ...
  if (!keyword) {
    return res
      .status(400)
      .json({ status: 'error', message: 'Keyword is required' })
  }
  ...
  try {
    ...
    if (articles.length === 0) {
      return res.status(404).json({
        status: 'error',
        message: 'No articles found matching the keyword',
      })
    }
    return res.json({ status: 'success', data: { articles } })
  } catch (error) {
    ...
    return res
      .status(500)
      .json({ status: 'error', message: 'Error searching for articles' })
  }
})
...
//   // 只會回傳單筆資料
//   const product = await Article.findByPk(id, {
//     raw: true, // 只需要資料表中資料
//   })
